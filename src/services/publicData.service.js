...
const getItemsData = async (ownerId) => {
        ...
        other_images AS (
            SELECT 
                item_id,
                json_agg(image_url) as image_urls  -- Changed to simple URL array
            FROM images
            WHERE ismain = false
            GROUP BY item_id
        )
        SELECT 
            ...
            json_agg(
                json_build_object(
                    ...
                    'other_images', COALESCE(oi.image_urls, '[]'::json),  
...
const getItemsData2 = async (ownerId) => {
    const query = `
        WITH item_tags AS (
            SELECT it.item_id, array_agg(json_build_object('id', t.id, 'name', t.name, 'color', t.color)) as tags
            FROM item_tag it
...
const getTags = async () => {
    ...
    if (result.rows.length === 0) {
        return {success: false, code: 404, message: 'No tags found for this restaurant'}
    }
